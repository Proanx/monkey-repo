...
spinInit({
    ">0.0.1": function () {  // 旧版锚点
        let hook = document.querySelector(".bottom-actions");
        if (!hook) {
            return false;
        }
        // 旧版适用的插件粉丝牌框框CSS
    ...
    ">1.2.8": function () {  // B站新版前端（SC和点赞独立大按钮）锚点
        let hook = document.querySelector(".icon-left-part-new");
        if (!hook) {
            return false;
        }
        // 原版的按钮CSS调整
        ...
        GM_addStyle(".chat-input-ctnr .medal-section.scripted{padding: 0 12px;height:56px;}");
        return hook;
    },
    ">1.2.10": function () {  // 26.5.9更新的新版本界面
        let hook = document.querySelector(".icon-right-part");
        if (!hook) {
            return false;
        }
        GM_addStyle(".dialog-ctnr.medal{top:-380px;left:8px}");  // 插件粉丝牌框框CSS
        return hook;
    }
}, 7000)
    .then((result) => main());
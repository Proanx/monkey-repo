...
(function () {
			...
			for (const node of mutationDetail.addedNodes) {
				...
				switch (node.classList[1]) {
					...
					case "danmaku-item":
						...
						// 表情弹幕改高清
						if (tempVar = node.querySelector(".emoticon")) {
							let img = tempVar.querySelector("img");
					...
					// 房间提示
					case "misc-msg":
						console.warn("房间提示：misc-msg", node);
						// 禁言
						if (node.classList[2] == "user-block") {
							// 用户名标红
	...
	let enterObserver = new MutationObserver(async function (mutationsList) {
		for (const mutationDetail of mutationsList) {
			...
			for (const node of mutationDetail.addedNodes) {
				...
				// 防止无限制增长
				if (chatBox.childElementCount > 200) {
					let chatHistory = document.querySelector("#chat-history-list");
					// 判断滚动条位置决定是否添加
					if (chatHistory.scrollTop > chatHistory.scrollHeight - 2 * chatHistory.offsetHeight) {
	...
	async function setFaceUrl(ele, uid) {
		...
		let res = await fetch(`https://api.bilibili.com/x/space/acc/info?mid=${uid}&jsonp=jsonp`);

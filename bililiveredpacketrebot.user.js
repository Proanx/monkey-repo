...
(function () {
            ...
            .then(json => {
                if (json.code != 0 || json.data.join_status != 1) {
                    ...
                    if (json.code == 1009109) {
                        showMessage(json.message, "warning", null, 50);
                        limitWarning[new Date().toLocaleDateString('zh')] = true;
                        return;
                    }
                    showMessage(json.message, "error", "抢红包失败", 100);
                } else {
                    showMessage(`坐等 ${message.data.sender_name} 的红包开奖<br>红包ID：${message.data.lot_id}`, "info", "啊哈哈哈哈哈哈，红包来咯", message.data.last_time * 10);
                }
            });
    }
    ...
    function radPacketWinner(message) {
        ...
        // showMessage("emmmmm啥也没中诶", "warning", "太惨了");
    }

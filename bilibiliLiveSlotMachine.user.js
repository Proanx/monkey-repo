...
(function () {
    ...
    var timeout, intervalCount = 0, interval = setInterval(() => {
        ...
        if (sideBar) {
            ...
            // 抽奖姬面板
            divElement.innerHTML = `<div class="gamble-panel m-auto dp-none p-fixed a-forwards"><div class="drag-bar"><div class="bar"></div></div><div class="title-ctnr p-relative"><h2 class="popup-title">抽奖设定</h2></div><div class="popup-content-ctnr"><div class="room-manager-ctnr"><div class="gamble-general-setting"><div class="input-wrap"><div><label class="input-lable" for="gamble-keyword">弹幕关键字:</label><input id="gamble-keyword" type="text" placeholder="支持正则表达式" class="link-input"></div><div><label class="input-lable" for="gamble-player-limit">人数限制:</label><input id="gamble-player-limit" type="number" placeholder="0为无上限" min="0" class="link-input"></div><div><label class="input-lable" for="gamble-bouns-count">奖品数:</label><input id="gamble-bouns-count" type="number" placeholder="1" min="0" class="link-input"></div></div><div class="input-wrap"><div><label class="input-lable" for="gamble-count_down">限时(秒):</label><input id="gamble-count_down" type="number" placeholder="0为手动停止" min="0" class="link-input"></div><div style="margin-top: 8px;"><input id="gamble-repeatable" type="checkbox" style="display: none;"><span id="gamble-repeatable-icon" class="check-icon pointer svg-icon v-middle checkbox-default"></span><label class="v-middle" for="gamble-repeatable" style="color:#666">可重复获奖</label></div><div class="gamble-btn"><button id="gamble-start-btn" class="bl-button bl-button-primary bl-button-size"><span class="txt">开始</span></button><button id="gamble-stop-btn" class="dp-none bl-button bl-button-error bl-button-size"><span class="txt">停止</span></button><button id="gamble-draw-btn" disabled class="gamble-btn-small bl-button bl-button-primary bl-button-size"><span class="txt">抽奖</span></button></div></div><div><hr class="gamble-progress-bar"></div><div class="player-list"><div id="biliTvLogo" class="dp-none link-progress-tv progress-tv"><button class="tv-clost-btn pointer">关闭</button><div class="text">小电视祈祷中</div><div class="result"></div></div><div class="dp-flex" style="justify-content: space-between;"><div class="tip">当前参与人数：<span class="player-count">0</span></div><div class="gamble-timer tip"></div><div class="tip" style="opacity:0">当前参与人数：0</div></div><table class="list"><thead><tr class="list-head"><th width="20%">用户名</th><th width="20%">UID</th><th width="15%">弹幕时间</th><th width="40%">弹幕内容</th></tr></thead><tbody class="player-list-body p-relative dp-block"></tbody></table></div></div></div></div><div id="gamble-panel-close-btn" title="关闭面板" class="close-btn p-absolute bg-center bg-no-repeat pointer t-center"><i class="icon-font icon-close"></i></div></div>`;
    ...
    function logPlayer(e) {
        ...
        if (item.classList && item.className.match(/chat-item danmaku-item/)) {
            ...
            temp.setAttribute("uid", "_" + item.getAttribute("data-uid"));
            temp.innerHTML = `<td width="20%">${item.getAttribute("data-uname")}</td><td width="20%">${item.getAttribute("data-uid")}</td><td width="15%">${new Date(ts).toLocaleTimeString('chinese', { hour12: false })}</td><td width="40%">${item.getAttribute("data-danmaku")}</td>`;
            tbody.append(temp);
            tbody.scrollTop += 26;
    ...
    function randomPick(num, allowRepeat) {
        ...
        for (let i = 1; i <= num; i++) {
            ...
            let resultInterval = setInterval(() => {
                ...
                let playerRow = document.querySelector(`.player-list-body .list-row[uid=_${uidList[random]}]`);
                tbody.scrollTop = 0;
                playerRow.classList.add("lucky");
                tbody.prepend(playerRow);
    ...
    function initTVCloseBtn() {
        let tvCloseBtn = document.querySelector(".tv-clost-btn");
        tvCloseBtn.onclick = () => {

...
(function () {
    ...
    function logPlayer(message) {
        ...
        if (playerLimit <= player.size || player.has(uid) || (keyWord && content.search(keyWord) < 0)) {
            return;
        }
    ...
    function startBtnFunc() {
        ...
        if (countDownTime) {
            ...
            progressBar.style.transition = `border ease-out .4s,width linear ${+countDownTime}s`;
    ...
    function logResult() {
        ...
        htmlText = `<summary><span>${new Date(result.timestamp).toLocaleString('chinese', { hour12: false })}</span><span class="f-right" data-ts="${result.timestamp}">${formatDate(Math.round(result.timestamp / 1000))}</span></summary><div class="result">`;
